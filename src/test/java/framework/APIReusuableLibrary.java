...
public class APIReusuableLibrary {
            ...
            switch (methodType) {
                case POST:
                    ...
                    if (postBody instanceof File) {/* File */
                        ...
                        else if (postBody instanceof Json) {/* JSON */
                            if (headersMap != null) {
                                response = RestAssured.
                                        given().
                                        contentType(contentType).
    ...
    public String registrationPayload(String strBirthday, String strGender, String strName, String strNatid, String strSalary, String strTax) {

        JsonObject objJSON = new JsonObject();
        objJSON.addProperty("birthday", strBirthday);
        objJSON.addProperty("gender", strGender);
        objJSON.addProperty("name", strName);
        objJSON.addProperty("natid", strNatid);
        objJSON.addProperty("salary", strSalary);
        objJSON.addProperty("tax", strTax);
        return objJSON.toString();
    }

    public String heroSerializationDataMapper(String strBirthday, String strGender, String strName, String strNatid, String strSalary, String strTax) {
        String json = null;
        try {
            HeroRecord objHR = new HeroRecord();
            objHR.setBirthday(strBirthday);
            objHR.setGender(strGender);
            objHR.setName(strName);
            objHR.setNatid(strNatid);
            objHR.setSalary(strSalary);
            objHR.setTax(strTax);

            json = MAPPER.writeValueAsString(objHR);
